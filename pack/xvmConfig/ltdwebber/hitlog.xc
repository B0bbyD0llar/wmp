 ...
 */
{
  "hitLog": {
    ...
    "defaultHeader":  "<font size='13'><strong>Webbers ModPack</strong></font>",
    ...
    "formatHistory": "<textformat leading='-4' tabstops='[20,50,90,190]'><font size='11'>\u00D7{{hitlog.n-player}}:</font><tab><font color='{{c:dmg-kind}}'>{{dmg}}</font><tab>| {{hitlog.dmg-player}}<tab>|<font color='{{c:vtype}}'>{{vehicle}}</font> <font face='xvm' size='19' color='#FF0000'>{{hitlog.dead}}</font><tab>|{{name%.15s~..}} <font alpha='#A0'>{{clan}}</font></textformat>"
  }
}
